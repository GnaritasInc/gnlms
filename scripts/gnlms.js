...
jQuery(document).ready(function ($) {
	...
	$("a.gnlms-form-submit").click(function (event) {
		event.preventDefault();
		ajaxFormSubmit.call($(this).parents("form").get(0));
		return false;
	});
	...
	$("a.gnlms-course-launch").click (function(event) {
		...
		if (!$("#gnlms-course-monitor").length) {
				...
				.load(
					gnlms.ajaxurl+"?"+$.param({action:"gnlms-launch-course", id:$(this).data("courseId")}),

					function (responseText, textStatus, XMLHttpRequest) {
						
						$(".gnlms-scorm-message", this).hide();
