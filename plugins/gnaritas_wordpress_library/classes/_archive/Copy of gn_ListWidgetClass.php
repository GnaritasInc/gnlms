...
function addSortSQL ($atts) {
	...
	function addForm ($atts, $code) {
		...
		$defAtts=array( "gn_page_id","page_student_id", "key", "title",  "view_key", "view_context_filter", "context_filter","select_key", "select_context_filter",  "save_action", "hide", "filters", "mode", "link", "linkclass", "hideid");
		...
		if ( true) {
			...
			if ($atts["filters"])
				...
				//$str.="<div class='container'><a class='form-submit button' href='#'>Update</a></div>";
...
function generateSQL($atts) {
	...
	//$sql="select id as 'ID', name as 'Name', 'Edit' from gn_district";
...
function listWidgetHeader($key,$atts,$type, $code="") {
	...
	$class.= $atts["postout"]?" listwidget_postout":"";
		...
		$str.=($atts["fullspan"])?'			<div class="full_span">':'';
...
function listWidgetFooter($key,$atts,$type) {
	...
	function gn_list ($atts) {
		error_log ("******* Ajax List" .$atts["mode"]);

		if ($atts["mode"]=="save") {

		// Do the action $atts["save_action"]

			error_log("****Saving:".$atts["save_action"]);
