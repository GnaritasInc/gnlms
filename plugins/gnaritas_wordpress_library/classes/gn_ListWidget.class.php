...
function addForm ($atts, $code) {
		...
		if ($atts["filters"] )
			...
			if ($atts["filters"])
				$str.="<div class='container gnlist-filter-update'><input style='float:left; margin-top:5px;' type='submit' value='Update'/></div>";
...
function retrieveData ($sql, $countSQL, $atts) {
		...
		// CTW 12-13-2012 added activeForm filter
		 if ($this->paginate ){
		 	$offset = ($this->getPostValue("offset")&& ($_POST["activeForm"]==$atts["key"]))?$this->getPostValue("offset"):0;
