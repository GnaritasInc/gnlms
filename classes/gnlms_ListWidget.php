...
function super ($methodName) {
	...
	function registerShortcodes () {
		...
		add_shortcode('gnlms_user_completed_courses', array(&$this, 'doUserCompletedCourseList'));
		add_shortcode('gnlms_user_available_courses', array(&$this, 'doUserAvailableCourseList'));
		add_shortcode('gnlms_available_courses', array(&$this, 'listAvailableCourses'));
	...
	function listAvailableCourses ($atts, $content, $code) {
		global $gnlms;
		$defaults = array("title"=>"");
		$atts = shortcode_atts($defaults, $atts);
		$atts["key"]= is_user_logged_in() ? "user_available_courses" : "available_courses";
		$atts["defaultsort"]="title";
		return $this->doCustomList("available-courses", $atts);
	}
	...
	function doUserCompletedCourseList ($atts, $content, $code) {
		$atts["key"] = "user_completed_courses";
		$atts["context_filter"]="context_user_id";	
		return $this->gn_short_list($atts, $content, $code);
	}
	...
	function listUserCurrentCourses ($atts, $content, $code) {
		...
		$atts["context_filter"]="context_user_id";
		$atts["defaultsort"] = "c.title";
		if (!array_key_exists("link", $atts)) $atts["link"] = "/course/";
